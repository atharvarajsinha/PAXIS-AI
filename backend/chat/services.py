...
def _enrich_roadmap_with_study_material(client, roadmap, model):
    ...
    for topic in _roadmap_topics(roadmap):
        ...
        yield {'status': f'Searching for {topic} tutorials...'}
        if settings.DEBUG:
...
def generate_learning_response(message, conversation_history=None, profile_context=''):
    ...
    if roadmap:
        # Yield the base roadmap immediately so it renders on the right side
        yield {'roadmap': roadmap}
        try:
            for chunk in _enrich_roadmap_with_study_material(client, roadmap, settings.GEMINI_MODEL):
                yield chunk
        except Exception as exc:
            if settings.DEBUG:
                logger.warning(
            ...
            # Re-yield the original if enrichment fails
            yield {'roadmap': roadmap}
    else:
        yield {'roadmap': None}
